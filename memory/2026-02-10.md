...
- 入庫/登記 Flow（Forms_病歷室入庫）需將 Create item 正確帶入 Forms 動態內容；不可把 `outputs('取得回應詳細資料')?['body/<key>']` 當純文字塞進 SharePoint 欄位。若用 code view，表達式需前綴 `@` 才會執行。
- 入庫 Flow 曾出現「寫兩筆、一筆空白」：Trigger 原本含 `splitOn: @triggerOutputs()?['body/value']`，已確認移除 splitOn 後的 trigger JSON（無 splitOn、僅 conditions:[]）為正確方向。
- 取件 Flow（病歷室取件）上線安全原則：Update item 僅更新取件相關欄位（Id、ReceivedAt=utcNow、Status=已取件、取件人），其餘欄位需移除以避免把 null/空值回寫洗掉資料。
