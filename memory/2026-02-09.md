...
### 2026-02-09 晚間補充（外部取件流程反覆除錯）
- `病歷室取件` 流程主幹已固定：
  - 當提交新的回應時 → 取得回應詳細資料 → 取得多個項目 → 條件(length=1) → 更新項目。
- 多次失敗主因確認：
  1) `取得多個項目` 的 OData `篩選查詢` 欄位被多次貼上造成「污染拼接」，出現殘字串（如 `...Status eq '已登記'ubstringof...`），導致無效運算式。
  2) 表單動態欄位 token 路徑不穩時會變空字串，造成查詢變成 `Title eq ''` 或 `substringof('', Title)`。
  3) OData 使用顯示名稱 `單位` 會報「欄不存在」，需用內部欄位名。
- 已確認條件節點本身語法正常（`length(body('取得多個項目')?['value']) = 1`），真正卡點多為上游回傳筆數非 1。
- 已確認 `更新項目` 的 `ID` 為 SharePoint 主鍵定位用途（更新必填），不是身份比對鍵。
- 使用者偏好業務快速流程，但需降低誤配；目前方向：`後3碼 + 單位 + Status=已登記`（唯一才更新），或改回完整病歷號碼比對。
- 重要發現：SharePoint 欄位格式 JSON 顯示 `病歷號碼` 內部名為 `$_x75c5__x6b77__x865f__x78bc_`（證實欄位存在）。

### 2026-02-09 22:15-22:32 取件流程最新狀態（高優先）
- 使用者回報 `取得多個項目` 錯誤：`欄 '_x75c5__x6b77__x865f__x78bc_' 不存在`，確認目前 OData 內部欄位命名路線不穩。
- 已決策改為「Get items 不做 OData 精準比對 + Data Operations 篩選陣列」的穩定方案，避免再被內部欄位名卡死。
- 流程目前已新增 `篩選陣列` 動作（位於 `取得多個項目` 與 `條件` 之間）。
- 已由助理代填 `篩選陣列` 的 `From`：`@body('取得多個項目')?['value']`。
- 目前阻塞點：Power Automate 出現 `Cannot edit in read-only editor`，導致進階模式貼 expression 操作中斷；需先關閉唯讀編輯器彈窗再繼續。
- `條件` 卡片 UI 仍可見第二列空白條件（新設計器 bug）；但程式碼邏輯已確認可用單一 equals 判斷，不必被空白列誤導。
- 待完成（下一步最短路徑）：
  1) 在 `篩選陣列` 進階模式貼上 and 條件（Status=已登記、單位相等、Title 後3碼比對）
  2) `條件` 改成 `length(body('篩選陣列')) = 1`
  3) `更新項目.id` 改成 `first(body('篩選陣列'))?['ID']`
